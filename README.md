...
Use the make file by simply typing ```make``` in the project directory. This directory will contain the “src” folder that has main and nodes inside. Make will generate the bin file that has the class files. 

## Execution

To run, go to the bin file that’s generated by the ```make``` command and type ```java main.Main <input file>``` when the ```<input file>``` is in the bin. The Main file is in the main folder, and the project runs inside the bin folder that contains all the executables and input files.
